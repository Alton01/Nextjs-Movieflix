...
export default function DetailsPopup ({ show, setShow }) {
  ...
  const { mediaDetails,
    setMediaDetails,
    similarMedias,
    setSimilarMedias, loggedInAccount, currentMediaInfoIdAndType, setCurrentMediaInfoIdAndType 
    ...
    useEffect(() => {
        ...
        if (currentMediaInfoIdAndType !== null) {
            async function getMediaDetails() {
                ...
                const findIndexOfClip = extractMediaDetails && extractMediaDetails.videos 
                && extractMediaDetails.videos.results
                && extractMediaDetails.videos.results.findIndex(
                (item) => item.type === "Clip" );    
    ...
    >
        ...
        >
            <div>
                ...
                <h2 className=" mt-7 mb-7 cursor-pointer text-sm font-semibold text-{#e5e5e5} transition-colors duration-200 hover:text-white md:text-2xl" >
                    More Like This
                </h2>
