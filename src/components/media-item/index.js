...
//FUNCTIONALITY FOR UPDATING FAVORITES IN MY-LIST PAGE AFTER ADDING TO FAV FROM PLAYING VIDEO
    ...
    // FUNCTIONALITY FOR ADDING TO FAVORITES
    ...
    async function handleAddToFavorites(item) {
        ...
        if (searchView) {
            ...
            const indexOfCurrentAddedMedia = updatedSimilarMedias.findIndex((item) => item.id === id);
            ...
            const findIndexOfCurrentMovie = currentMovieArrayFromRowItem.findIndex((item) => item.id === id)
    ...
    return ( 
        ...
        >
            <div className='relative cardWrapper h-28 min-w-[180px] cursor-pointer md:h-36 md:min-w-[260px] transform transition duration-500 hover:scale-110 hover:z-[999]'>
                ...
                <button onClick={media?.addedToFavorites
